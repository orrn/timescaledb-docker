...
RETURNS fdw_handler
AS '$libdir/timescaledb-2.8.1', 'ts_timescaledb_fdw_handler'
LANGUAGE C STRICT;

CREATE FUNCTION @extschema@.timescaledb_fdw_validator(text[], oid)
RETURNS void
AS '$libdir/timescaledb-2.8.1', 'ts_timescaledb_fdw_validator'
LANGUAGE C STRICT;
...
AS '$libdir/timescaledb-2.8.1', 'ts_hypertable_insert_blocker' LANGUAGE C;
...
AS '$libdir/timescaledb-2.8.1', 'ts_continuous_agg_invalidation_trigger' LANGUAGE C;
...
AS '$libdir/timescaledb-2.8.1', 'ts_hypertable_set_integer_now_func'
...
) RETURNS BIGINT AS '$libdir/timescaledb-2.8.1', 'ts_calculate_chunk_interval' LANGUAGE C;

-- Get the status of the chunk
CREATE FUNCTION _timescaledb_internal.chunk_status(REGCLASS) RETURNS INT
AS '$libdir/timescaledb-2.8.1', 'ts_chunk_status' LANGUAGE C;
...
AS '$libdir/timescaledb-2.8.1', 'ts_chunks_in' LANGUAGE C STABLE STRICT PARALLEL SAFE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_chunk_id_from_relid' LANGUAGE C STABLE STRICT PARALLEL SAFE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_chunk_show' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_chunk_create' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_chunk_set_default_data_node' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_chunk_get_relstats' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_chunk_get_colstats' LANGUAGE C VOLATILE;
...
CREATE FUNCTION _timescaledb_internal.create_chunk_table(
       ...
       schema_name NAME,
       table_name NAME)
RETURNS BOOL AS '$libdir/timescaledb-2.8.1', 'ts_chunk_create_empty_table' LANGUAGE C VOLATILE;

CREATE FUNCTION _timescaledb_internal.freeze_chunk(
   chunk REGCLASS)
RETURNS BOOL AS '$libdir/timescaledb-2.8.1', 'ts_chunk_freeze_chunk' LANGUAGE C VOLATILE;

CREATE FUNCTION _timescaledb_internal.unfreeze_chunk(
   chunk REGCLASS)
RETURNS BOOL AS '$libdir/timescaledb-2.8.1', 'ts_chunk_unfreeze_chunk' LANGUAGE C VOLATILE;
...
CREATE FUNCTION _timescaledb_internal.drop_chunk(
   chunk REGCLASS)
RETURNS BOOL AS '$libdir/timescaledb-2.8.1', 'ts_chunk_drop_single_chunk' LANGUAGE C VOLATILE;
...
CREATE FUNCTION _timescaledb_internal.attach_osm_table_chunk(
   hypertable REGCLASS,
   chunk REGCLASS)
RETURNS BOOL AS '$libdir/timescaledb-2.8.1', 'ts_chunk_attach_osm_table_chunk' LANGUAGE C VOLATILE;
-- This file and its contents are licensed under the Apache License 2.0.
-- Please see the included NOTICE for copyright information and
-- LICENSE-APACHE for a copy of the license.
...
AS '$libdir/timescaledb-2.8.1', 'ts_data_node_ping' LANGUAGE C VOLATILE;

CREATE FUNCTION _timescaledb_internal.remote_txn_heal_data_node(foreign_server_oid oid)
RETURNS INT
AS '$libdir/timescaledb-2.8.1', 'ts_remote_txn_heal_data_node'
LANGUAGE C STRICT;
-- This file and its contents are licensed under the Apache License 2.0.
-- Please see the included NOTICE for copyright information and
-- LICENSE-APACHE for a copy of the license.
...
AS '$libdir/timescaledb-2.8.1', 'ts_chunk_index_clone' LANGUAGE C VOLATILE STRICT;

CREATE FUNCTION _timescaledb_internal.chunk_index_replace(chunk_index_oid_old OID, chunk_index_oid_new OID) RETURNS VOID
AS '$libdir/timescaledb-2.8.1', 'ts_chunk_index_replace' LANGUAGE C VOLATILE STRICT;

CREATE FUNCTION _timescaledb_internal.create_chunk_replica_table(
    chunk REGCLASS,
    data_node_name NAME
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_chunk_create_replica_table' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_chunk_drop_replica' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_data_node_block_new_chunks' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_data_node_allow_new_chunks' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_move_chunk_proc' LANGUAGE C;
...
CREATE PROCEDURE timescaledb_experimental.copy_chunk(
    ...
    destination_node NAME = NULL,
    operation_id NAME = NULL)
AS '$libdir/timescaledb-2.8.1', 'ts_copy_chunk_proc' LANGUAGE C;

CREATE FUNCTION timescaledb_experimental.subscription_exec(
    subscription_command TEXT
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_subscription_exec' LANGUAGE C VOLATILE;
...
CREATE PROCEDURE timescaledb_experimental.cleanup_copy_chunk_operation(
    operation_id NAME)
AS '$libdir/timescaledb-2.8.1', 'ts_copy_chunk_cleanup_proc' LANGUAGE C;
-- This file and its contents are licensed under the Apache License 2.0.
-- Please see the included NOTICE for copyright information and
-- LICENSE-APACHE for a copy of the license.
...
CREATE FUNCTION _timescaledb_internal.to_unix_microseconds(ts TIMESTAMPTZ) RETURNS BIGINT
    AS '$libdir/timescaledb-2.8.1', 'ts_pg_timestamp_to_unix_microseconds' LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION _timescaledb_internal.to_timestamp(unixtime_us BIGINT) RETURNS TIMESTAMPTZ
    AS '$libdir/timescaledb-2.8.1', 'ts_pg_unix_microseconds_to_timestamp' LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION _timescaledb_internal.to_timestamp_without_timezone(unixtime_us BIGINT)
  RETURNS TIMESTAMP
  AS '$libdir/timescaledb-2.8.1', 'ts_pg_unix_microseconds_to_timestamp'
  LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION _timescaledb_internal.to_date(unixtime_us BIGINT)
  RETURNS DATE
  AS '$libdir/timescaledb-2.8.1', 'ts_pg_unix_microseconds_to_date'
  LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION _timescaledb_internal.to_interval(unixtime_us BIGINT) RETURNS INTERVAL
    AS '$libdir/timescaledb-2.8.1', 'ts_pg_unix_microseconds_to_interval' LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
...
RETURNS BIGINT AS '$libdir/timescaledb-2.8.1', 'ts_time_to_internal' LANGUAGE C VOLATILE STRICT;

CREATE FUNCTION _timescaledb_internal.cagg_watermark(hypertable_id INTEGER)
RETURNS INT8 AS '$libdir/timescaledb-2.8.1', 'ts_continuous_agg_watermark' LANGUAGE C STABLE STRICT;

CREATE FUNCTION _timescaledb_internal.subtract_integer_from_now( hypertable_relid REGCLASS, lag INT8 )
RETURNS INT8 AS '$libdir/timescaledb-2.8.1', 'ts_subtract_integer_from_now' LANGUAGE C STABLE STRICT;
-- This file and its contents are licensed under the Apache License 2.0.
-- Please see the included NOTICE for copyright information and
-- LICENSE-APACHE for a copy of the license.
...
CREATE FUNCTION _timescaledb_internal.get_partition_for_key(val anyelement)
    RETURNS int
    AS '$libdir/timescaledb-2.8.1', 'ts_get_partition_for_key' LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION _timescaledb_internal.get_partition_hash(val anyelement)
    RETURNS int
    AS '$libdir/timescaledb-2.8.1', 'ts_get_partition_hash' LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION _timescaledb_internal.get_time_type(hypertable_id INTEGER)
    RETURNS OID
    AS '$libdir/timescaledb-2.8.1', 'ts_hypertable_get_time_type' LANGUAGE C STABLE STRICT;
-- This file and its contents are licensed under the Apache License 2.0.
-- Please see the included NOTICE for copyright information and
-- LICENSE-APACHE for a copy of the license.
...
) RETURNS TABLE(hypertable_id INT, schema_name NAME, table_name NAME, created BOOL) AS '$libdir/timescaledb-2.8.1', 'ts_hypertable_create' LANGUAGE C VOLATILE;
...
CREATE FUNCTION @extschema@.create_distributed_hypertable(
    ...
    replication_factor      INTEGER = NULL,
    data_nodes              NAME[] = NULL
) RETURNS TABLE(hypertable_id INT, schema_name NAME, table_name NAME, created BOOL) AS '$libdir/timescaledb-2.8.1', 'ts_hypertable_distributed_create' LANGUAGE C VOLATILE;
...
) RETURNS RECORD AS '$libdir/timescaledb-2.8.1', 'ts_chunk_adaptive_set' LANGUAGE C VOLATILE;
...
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_dimension_set_interval' LANGUAGE C VOLATILE;

CREATE FUNCTION @extschema@.set_number_partitions(
    hypertable              REGCLASS,
    number_partitions       INTEGER,
    dimension_name          NAME = NULL
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_dimension_set_num_slices' LANGUAGE C VOLATILE;
...
) RETURNS SETOF TEXT AS '$libdir/timescaledb-2.8.1', 'ts_chunk_drop_chunks'
...
CREATE FUNCTION @extschema@.show_chunks(
    ...
    older_than             "any" = NULL,
    newer_than             "any" = NULL
) RETURNS SETOF REGCLASS AS '$libdir/timescaledb-2.8.1', 'ts_chunk_show_chunks'
...
AS '$libdir/timescaledb-2.8.1', 'ts_dimension_add' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_tablespace_attach' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_tablespace_detach' LANGUAGE C VOLATILE;

CREATE FUNCTION @extschema@.detach_tablespaces(hypertable REGCLASS) RETURNS INTEGER
AS '$libdir/timescaledb-2.8.1', 'ts_tablespace_detach_all_from_hypertable' LANGUAGE C VOLATILE;

CREATE FUNCTION @extschema@.show_tablespaces(hypertable REGCLASS) RETURNS SETOF NAME
AS '$libdir/timescaledb-2.8.1', 'ts_tablespace_show' LANGUAGE C VOLATILE STRICT;
...
AS '$libdir/timescaledb-2.8.1', 'ts_data_node_add' LANGUAGE C VOLATILE;
...
) RETURNS BOOLEAN AS '$libdir/timescaledb-2.8.1', 'ts_data_node_delete' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_data_node_attach' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_data_node_detach' LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_distributed_exec' LANGUAGE C;
...
AS '$libdir/timescaledb-2.8.1', 'ts_create_distributed_restore_point' LANGUAGE C VOLATILE STRICT;
...
AS '$libdir/timescaledb-2.8.1', 'ts_hypertable_distributed_set_replication_factor' LANGUAGE C VOLATILE;
...
CREATE PROCEDURE @extschema@.refresh_continuous_aggregate(
    ...
    window_start             "any",
    window_end               "any"
) LANGUAGE C AS '$libdir/timescaledb-2.8.1', 'ts_continuous_agg_refresh';
-- This file and its contents are licensed under the Apache License 2.0.
-- Please see the included NOTICE for copyright information and
-- LICENSE-APACHE for a copy of the license.
...
AS '$libdir/timescaledb-2.8.1', 'ts_timescaledb_process_ddl_event' LANGUAGE C;
...
CREATE FUNCTION @extschema@.time_bucket(bucket_width INTERVAL, ts TIMESTAMP) RETURNS TIMESTAMP
	AS '$libdir/timescaledb-2.8.1', 'ts_timestamp_bucket' LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
...
CREATE FUNCTION @extschema@.time_bucket(bucket_width INTERVAL, ts TIMESTAMPTZ) RETURNS TIMESTAMPTZ
	AS '$libdir/timescaledb-2.8.1', 'ts_timestamptz_bucket' LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
...
CREATE FUNCTION @extschema@.time_bucket(bucket_width INTERVAL, ts DATE) RETURNS DATE
	AS '$libdir/timescaledb-2.8.1', 'ts_date_bucket' LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
...
CREATE FUNCTION @extschema@.time_bucket(bucket_width INTERVAL, ts TIMESTAMPTZ, timezone TEXT, origin TIMESTAMPTZ DEFAULT NULL, "offset" INTERVAL DEFAULT NULL) RETURNS TIMESTAMPTZ
	AS '$libdir/timescaledb-2.8.1', 'ts_timestamptz_timezone_bucket' LANGUAGE C IMMUTABLE PARALLEL SAFE;

-- bucketing of int
CREATE FUNCTION @extschema@.time_bucket(bucket_width SMALLINT, ts SMALLINT) RETURNS SMALLINT
	AS '$libdir/timescaledb-2.8.1', 'ts_int16_bucket' LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
CREATE FUNCTION @extschema@.time_bucket(bucket_width INT, ts INT) RETURNS INT
	AS '$libdir/timescaledb-2.8.1', 'ts_int32_bucket' LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
CREATE FUNCTION @extschema@.time_bucket(bucket_width BIGINT, ts BIGINT) RETURNS BIGINT
	AS '$libdir/timescaledb-2.8.1', 'ts_int64_bucket' LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
...
CREATE FUNCTION timescaledb_experimental.time_bucket_ng(bucket_width INTERVAL, ts DATE) RETURNS DATE
    AS '$libdir/timescaledb-2.8.1', 'ts_time_bucket_ng_date' LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
...
CREATE FUNCTION timescaledb_experimental.time_bucket_ng(bucket_width INTERVAL, ts TIMESTAMP) RETURNS TIMESTAMP
    AS '$libdir/timescaledb-2.8.1', 'ts_time_bucket_ng_timestamp' LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
...
CREATE FUNCTION timescaledb_experimental.time_bucket_ng(bucket_width INTERVAL, ts TIMESTAMPTZ, timezone TEXT) RETURNS TIMESTAMPTZ
    AS '$libdir/timescaledb-2.8.1', 'ts_time_bucket_ng_timezone' LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
CREATE FUNCTION timescaledb_experimental.time_bucket_ng(bucket_width INTERVAL, ts TIMESTAMPTZ, origin TIMESTAMPTZ, timezone TEXT) RETURNS TIMESTAMPTZ
    AS '$libdir/timescaledb-2.8.1', 'ts_time_bucket_ng_timezone_origin' LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
...
CREATE FUNCTION timescaledb_experimental.time_bucket_ng(bucket_width INTERVAL, ts TIMESTAMPTZ) RETURNS TIMESTAMPTZ
    AS '$libdir/timescaledb-2.8.1', 'ts_time_bucket_ng_timestamptz' LANGUAGE C STABLE PARALLEL SAFE STRICT;
...
CREATE FUNCTION _timescaledb_internal.get_git_commit()
    ...
    AS '$libdir/timescaledb-2.8.1', 'ts_get_git_commit' LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
...
CREATE FUNCTION _timescaledb_internal.get_os_info()
    ...
    AS '$libdir/timescaledb-2.8.1', 'ts_get_os_info' LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION _timescaledb_internal.tsl_loaded() RETURNS BOOLEAN
AS '$libdir/timescaledb-2.8.1', 'ts_tsl_loaded' LANGUAGE C;
-- This file and its contents are licensed under the Apache License 2.0.
-- Please see the included NOTICE for copyright information and
-- LICENSE-APACHE for a copy of the license.
...
AS '$libdir/timescaledb-2.8.1', 'ts_relation_size' LANGUAGE C VOLATILE;
...
RETURNS TABLE (
    ...
    toast_bytes     bigint,
    total_bytes     bigint)
AS '$libdir/timescaledb-2.8.1', 'ts_dist_remote_hypertable_info' LANGUAGE C VOLATILE STRICT;
...
RETURNS TABLE (
    ...
    toast_bytes     bigint,
    total_bytes     bigint)
AS '$libdir/timescaledb-2.8.1', 'ts_dist_remote_chunk_info' LANGUAGE C VOLATILE STRICT;
...
AS '$libdir/timescaledb-2.8.1' , 'ts_dist_remote_compressed_chunk_info' LANGUAGE C VOLATILE STRICT;
...
AS '$libdir/timescaledb-2.8.1' , 'ts_dist_remote_hypertable_index_info' LANGUAGE C VOLATILE STRICT;
...
RETURNS INTEGER[]
AS '$libdir/timescaledb-2.8.1', 'ts_hist_finalfunc'
LANGUAGE C IMMUTABLE PARALLEL SAFE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_uuid_generate' LANGUAGE C VOLATILE STRICT;
...
AS '$libdir/timescaledb-2.8.1', 'ts_dist_set_id' LANGUAGE C VOLATILE STRICT;

CREATE FUNCTION _timescaledb_internal.set_peer_dist_id(dist_id UUID) RETURNS BOOL
AS '$libdir/timescaledb-2.8.1', 'ts_dist_set_peer_id' LANGUAGE C VOLATILE STRICT;
...
AS '$libdir/timescaledb-2.8.1', 'ts_dist_validate_as_data_node' LANGUAGE C VOLATILE STRICT;
...
RETURNS TABLE (
    ...
    processing          boolean,
    invalidated         boolean)
AS '$libdir/timescaledb-2.8.1', 'ts_remote_connection_cache_show' LANGUAGE C VOLATILE STRICT;
-- This file and its contents are licensed under the Apache License 2.0.
-- Please see the included NOTICE for copyright information and
-- LICENSE-APACHE for a copy of the license.
...
CREATE FUNCTION @extschema@.time_bucket_gapfill(bucket_width SMALLINT, ts SMALLINT, start SMALLINT=NULL, finish SMALLINT=NULL) RETURNS SMALLINT
	AS '$libdir/timescaledb-2.8.1', 'ts_gapfill_int16_bucket' LANGUAGE C VOLATILE PARALLEL SAFE;

CREATE FUNCTION @extschema@.time_bucket_gapfill(bucket_width INT, ts INT, start INT=NULL, finish INT=NULL) RETURNS INT
	AS '$libdir/timescaledb-2.8.1', 'ts_gapfill_int32_bucket' LANGUAGE C VOLATILE PARALLEL SAFE;
...
CREATE FUNCTION @extschema@.time_bucket_gapfill(bucket_width BIGINT, ts BIGINT, start BIGINT=NULL, finish BIGINT=NULL) RETURNS BIGINT
	AS '$libdir/timescaledb-2.8.1', 'ts_gapfill_int64_bucket' LANGUAGE C VOLATILE PARALLEL SAFE;
...
CREATE FUNCTION @extschema@.time_bucket_gapfill(bucket_width INTERVAL, ts DATE, start DATE=NULL, finish DATE=NULL) RETURNS DATE
	AS '$libdir/timescaledb-2.8.1', 'ts_gapfill_date_bucket' LANGUAGE C VOLATILE PARALLEL SAFE;
...
CREATE FUNCTION @extschema@.time_bucket_gapfill(bucket_width INTERVAL, ts TIMESTAMP, start TIMESTAMP=NULL, finish TIMESTAMP=NULL) RETURNS TIMESTAMP
	AS '$libdir/timescaledb-2.8.1', 'ts_gapfill_timestamp_bucket' LANGUAGE C VOLATILE PARALLEL SAFE;
...
CREATE FUNCTION @extschema@.time_bucket_gapfill(bucket_width INTERVAL, ts TIMESTAMPTZ, start TIMESTAMPTZ=NULL, finish TIMESTAMPTZ=NULL) RETURNS TIMESTAMPTZ
	AS '$libdir/timescaledb-2.8.1', 'ts_gapfill_timestamptz_bucket' LANGUAGE C VOLATILE PARALLEL SAFE;

-- locf function
CREATE FUNCTION @extschema@.locf(value ANYELEMENT, prev ANYELEMENT=NULL, treat_null_as_missing BOOL=false) RETURNS ANYELEMENT
	AS '$libdir/timescaledb-2.8.1', 'ts_gapfill_marker' LANGUAGE C VOLATILE PARALLEL SAFE;
...
CREATE FUNCTION @extschema@.reorder_chunk(
    ...
    index REGCLASS=NULL,
    verbose BOOLEAN=FALSE
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_reorder_chunk' LANGUAGE C VOLATILE;
...
CREATE FUNCTION @extschema@.move_chunk(
    ...
    verbose BOOLEAN=FALSE
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_move_chunk' LANGUAGE C VOLATILE;
...
CREATE FUNCTION _timescaledb_internal.create_compressed_chunk(
    ...
    numrows_pre_compression BIGINT,
    numrows_post_compression BIGINT
) RETURNS REGCLASS AS '$libdir/timescaledb-2.8.1', 'ts_create_compressed_chunk' LANGUAGE C STRICT VOLATILE;

CREATE FUNCTION @extschema@.compress_chunk(
    uncompressed_chunk REGCLASS,
    if_not_compressed BOOLEAN = false
) RETURNS REGCLASS AS '$libdir/timescaledb-2.8.1', 'ts_compress_chunk' LANGUAGE C STRICT VOLATILE;

CREATE FUNCTION @extschema@.decompress_chunk(
    uncompressed_chunk REGCLASS,
    if_compressed BOOLEAN = false
) RETURNS REGCLASS AS '$libdir/timescaledb-2.8.1', 'ts_decompress_chunk' LANGUAGE C STRICT VOLATILE;
...
RETURNS BYTEA AS '$libdir/timescaledb-2.8.1', 'ts_partialize_agg' LANGUAGE C STABLE PARALLEL SAFE;
...
RETURNS internal
AS '$libdir/timescaledb-2.8.1', 'ts_finalize_agg_sfunc'
LANGUAGE C IMMUTABLE PARALLEL SAFE;

CREATE FUNCTION _timescaledb_internal.finalize_agg_ffunc(
tstate internal, aggfn TEXT, inner_agg_collation_schema NAME, inner_agg_collation_name NAME, inner_agg_input_types NAME[][], inner_agg_serialized_state BYTEA, return_type_dummy_val ANYELEMENT)
RETURNS anyelement
AS '$libdir/timescaledb-2.8.1', 'ts_finalize_agg_ffunc'
LANGUAGE C IMMUTABLE PARALLEL SAFE;
...
) RETURNS INTEGER AS '$libdir/timescaledb-2.8.1', 'ts_job_add' LANGUAGE C VOLATILE;

CREATE FUNCTION @extschema@.delete_job(job_id INTEGER) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_job_delete' LANGUAGE C VOLATILE STRICT;
CREATE PROCEDURE @extschema@.run_job(job_id INTEGER) AS '$libdir/timescaledb-2.8.1', 'ts_job_run' LANGUAGE C;
...
AS '$libdir/timescaledb-2.8.1', 'ts_job_alter'
LANGUAGE C VOLATILE;

CREATE FUNCTION _timescaledb_internal.alter_job_set_hypertable_id(
    job_id INTEGER,
    hypertable REGCLASS )
RETURNS INTEGER AS '$libdir/timescaledb-2.8.1', 'ts_job_alter_set_hypertable_id'
LANGUAGE C VOLATILE;
-- This file and its contents are licensed under the Apache License 2.0.
-- Please see the included NOTICE for copyright information and
-- LICENSE-APACHE for a copy of the license.
...
RETURNS INTEGER AS '$libdir/timescaledb-2.8.1', 'ts_policy_retention_add'
LANGUAGE C VOLATILE;
...
AS '$libdir/timescaledb-2.8.1', 'ts_policy_retention_remove'
LANGUAGE C VOLATILE STRICT;
...
AS '$libdir/timescaledb-2.8.1', 'ts_policy_reorder_add'
LANGUAGE C VOLATILE STRICT;

CREATE FUNCTION @extschema@.remove_reorder_policy(hypertable REGCLASS, if_exists BOOL = false) RETURNS VOID
AS '$libdir/timescaledb-2.8.1', 'ts_policy_reorder_remove'
LANGUAGE C VOLATILE STRICT;
...
RETURNS INTEGER
AS '$libdir/timescaledb-2.8.1', 'ts_policy_compression_add'
...
AS '$libdir/timescaledb-2.8.1', 'ts_policy_compression_remove'
LANGUAGE C VOLATILE STRICT;
...
RETURNS INTEGER
AS '$libdir/timescaledb-2.8.1', 'ts_policy_refresh_cagg_add'
LANGUAGE C VOLATILE;
...
RETURNS VOID
AS '$libdir/timescaledb-2.8.1', 'ts_policy_refresh_cagg_remove'
LANGUAGE C VOLATILE;
...
CREATE FUNCTION timescaledb_experimental.add_policies(
    ...
    drop_after "any" = NULL)
RETURNS BOOL
AS '$libdir/timescaledb-2.8.1', 'ts_policies_add'
LANGUAGE C VOLATILE;
...
RETURNS BOOL
AS '$libdir/timescaledb-2.8.1', 'ts_policies_remove'
LANGUAGE C VOLATILE;
...
CREATE FUNCTION timescaledb_experimental.remove_all_policies(
    ...
    if_exists BOOL = false)
RETURNS BOOL
AS '$libdir/timescaledb-2.8.1', 'ts_policies_remove_all'
LANGUAGE C VOLATILE;
...
CREATE FUNCTION timescaledb_experimental.alter_policies(
    ...
    drop_after "any" = NULL)
RETURNS BOOL
AS '$libdir/timescaledb-2.8.1', 'ts_policies_alter'
LANGUAGE C VOLATILE;
...
CREATE FUNCTION timescaledb_experimental.show_policies(
    relation REGCLASS)
RETURNS SETOF JSONB
AS '$libdir/timescaledb-2.8.1', 'ts_policies_show'
LANGUAGE C  VOLATILE;
-- This file and its contents are licensed under the Apache License 2.0.
-- Please see the included NOTICE for copyright information and
-- LICENSE-APACHE for a copy of the license.
...
CREATE FUNCTION _timescaledb_internal.invalidation_cagg_log_add_entry(
    ...
    start_time BIGINT,
    end_time BIGINT
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_invalidation_cagg_log_add_entry' LANGUAGE C STRICT VOLATILE;
...
CREATE FUNCTION _timescaledb_internal.invalidation_hyper_log_add_entry(
    ...
    start_time BIGINT,
    end_time BIGINT
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_invalidation_hyper_log_add_entry' LANGUAGE C STRICT VOLATILE;

-- raw_hypertable_id - The hypertable ID of the original distributed hypertable in the Access Node
CREATE FUNCTION _timescaledb_internal.hypertable_invalidation_log_delete(
    raw_hypertable_id INTEGER
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_hypertable_invalidation_log_delete' LANGUAGE C STRICT VOLATILE;

-- mat_hypertable_id - The hypertable ID of the CAGG materialized hypertable in the Access Node
CREATE FUNCTION _timescaledb_internal.materialization_invalidation_log_delete(
    mat_hypertable_id INTEGER
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_materialization_invalidation_log_delete' LANGUAGE C STRICT VOLATILE;

-- raw_hypertable_id - The hypertable ID of the original distributed hypertable in the Access Node
CREATE FUNCTION _timescaledb_internal.drop_dist_ht_invalidation_trigger(
    raw_hypertable_id INTEGER
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_drop_dist_ht_invalidation_trigger' LANGUAGE C STRICT VOLATILE;
...
) RETURNS VOID AS '$libdir/timescaledb-2.8.1', 'ts_invalidation_process_hypertable_log' LANGUAGE C STRICT VOLATILE;
...
) RETURNS RECORD AS '$libdir/timescaledb-2.8.1', 'ts_invalidation_process_cagg_log' LANGUAGE C STRICT VOLATILE;
...
CREATE FUNCTION @extschema@.get_telemetry_report() RETURNS jsonb
    AS '$libdir/timescaledb-2.8.1', 'ts_telemetry_get_report_jsonb'
	LANGUAGE C STABLE PARALLEL SAFE;
